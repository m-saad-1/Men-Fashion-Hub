...
<div class="address-modal" id="addressModal">
   ...
   try {
       ...
       const response = await fetch(`${this.apiBaseUrl}/auth/validate_session.php`, {
           credentials: 'include'
       });
       ...
       // If not authenticated, clear cart and redirect to login
       this.clearUserCart();
       ...
       window.location.href = 'login.php';
            ...
            loadWishlistItems: async function() {
                ...
                try {
                    const response = await fetch(`${this.apiBaseUrl}/wishlist/get_wishlist.php`, {
                        credentials: 'include'
                    });
...
loadAddresses: async function() {
    ...
    try {
        const response = await fetch(`${this.apiBaseUrl}/users/get_addresses.php`, {
            credentials: 'include'
        });
        ...
        if (!this.currentUser.addresses) {
            ...
            loadUserCart: async function() {
                ...
                try {
                    const response = await fetch(`${this.apiBaseUrl}/cart/get_cart.php`, {
                        credentials: 'include'
                    });
                    ...
                    if (response.ok) {
                        ...
                        if (data.status === 'success') {
                            ...
                            // Update cart count
                            const cartCount = dbCart.reduce((total, item) => total + item.quantity, 0);
                            updateCartCount();
                        }
                    }
                } catch (error) {
...
menuLinks.forEach(link => {
    ...
    try {
        const response = await fetch(`${this.apiBaseUrl}/users/update_profile.php`, {
            method: 'POST',
            headers: {
        ...
        async function loadOrders() {
            ...
            try {
                const response = await fetch(`${auth.apiBaseUrl}/orders/get_orders.php`, {
                    credentials: 'include'
                });
                                ...
                                ${order.items.map(item => `
                                    <div class="detail-item">
                                        ...
                                        <div class="item-info">
                                            ...
                                            <div class="item-price">$${parseFloat(item.price || item.product_price || 0).toFixed(2)} Ã— ${item.quantity || 1}</div>
                                        </div>
        ...
        async function cancelOrder(orderId) {
            if (confirm('Are you sure you want to cancel this order?')) {
                try {
                    const response = await fetch(`${auth.apiBaseUrl}/orders/cancel_order.php`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
        ...
        function renderWishlistItems() {
            ...
            // Map the user's wishlist items
            const wishlistProducts = auth.currentUser.wishlist.map(wishlistItem => {
                if (wishlistItem.title && wishlistItem.price) {
                    return wishlistItem;
                }
...
function openAddressModal(type) {
  ...
  async function saveAddress(type, addressData) {
    try {
        const apiUrl = `${auth.apiBaseUrl}/users/save_addresses.php`;
